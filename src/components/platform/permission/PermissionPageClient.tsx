...
export function PermissionPageClient() {
    ...
    const [workspacePermission, setWorkspacePermission] =
        useState<WorkspacePermissionResponse | null>(null);
    ...
    const selectedStaff =
        filteredStaff.find((staff) => staff.id === selectedStaffId) || filteredStaff[0] || null;
        ...
        const fetchWorkspacePermission = async () => {
            ...
            try {
                ...
                const response = await fetch(
                    `/api/platform/permission/workspaces/${selectedStaff.workspaceId}`,
                    {
                        cache: 'no-store',
                    },
                );
        ...
        if (!response.ok) {
            throw new Error(
                (data as { message?: string } | null)?.message ||
                    'Failed to update workspace limits',
            );
        }
    ...
    useEffect(() => {
        async function fetchSites() {
            ...
            const res = await fetch(
                `/api/platform/permission/workspaces/${selectedStaff.workspaceId}/sites`,
                {
                    cache: 'no-store',
                },
            );
                ...
                <section className={styles.detailCard}>
                    {!selectedStaff || !accessProfile ? (
                        <div className={styles.emptyState}>
                            Select a tenant to view plan access.
                        </div>
                    ) : (
                        <>
                            ...
                            <div className={styles.tabCard}>
                                ...
                                >
                                    ...
                                    >
                                        {!selectedStaff.workspaceId ? (
                                            ...
                                            <button
                                                ...
                                                onClick={handleStartEditWorkspace}
                                            >
                                    ...
                                    {!selectedStaff.workspaceId ? (
                                        ...
                                        >
                                            This user does not have a workspace yet. Create one
                                            first so you can apply real limits, plans, and
                                            permissions.
                                        </div>
                                    ) : (
                                        <div
                                ...
                                {permissionError ? (
                                    <div className={styles.emptyState}>{permissionError}</div>
                                ) : null}
