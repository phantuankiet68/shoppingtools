...
import styles from '@/styles/platform/menus/modalAddMenus.module.css';
...
export function ModalAddMenus({
    ...
    const [form, setForm] = useState<FormState>(() =>
        createInitialForm(defaultSiteId, defaultArea),
    );
        ...
        async function fetchSites() {
            try {
                ...
                const data = (await response
                    .json()
                    .catch(() => null)) as SiteOptionsResponse | null;
    ...
    function handleTitleChange(value: string) {
        setForm((prev) => {
            const nextPath = didUserEditPathRef.current
                ? prev.path
                : buildPathFromTitle(prev.area, value);
    ...
    function handleAreaChange(nextArea: MenuArea) {
        setForm((prev) => {
            const nextPath = didUserEditPathRef.current
                ? prev.path
                : buildPathFromTitle(nextArea, prev.title);
                    ...
                    <div className={styles.grid}>
                        <label className={styles.field}>
                            ...
                            {effectiveSiteOptions.length > 0 ? (
                                <select
                                    value={form.siteId}
                                    onChange={(event) =>
                                        updateFormField('siteId', event.target.value)
                                    }
                        ...
                        <label className={styles.field}>
                            ...
                            <select
                                value={form.area}
                                onChange={(event) =>
                                    handleAreaChange(event.target.value as MenuArea)
                                }
                                className={styles.control}
                                disabled={submitting}
                            >
                        ...
                        <label className={styles.field}>
                            ...
                            <select
                                value={form.parentId}
                                onChange={(event) =>
                                    updateFormField('parentId', event.target.value)
                                }
                                className={styles.control}
                                disabled={submitting}
                            >
                    ...
                    <div className={styles.inlineMeta}>
                        ...
                        <label className={styles.visibilityToggle}>
                            <input
                                ...
                                onChange={(event) =>
                                    updateFormField('visible', event.target.checked)
                                }
                                disabled={submitting}
                            />
