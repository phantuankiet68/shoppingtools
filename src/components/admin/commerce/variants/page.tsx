...
function uiToDbCreatePayload(
    data: VariantFormState & { productId: string; siteId: string },
...
export default function VariantsPage() {
    ...
    const productVariants = useMemo(() => {
        ...
        return variants
            ...
            .filter((variant) => {
                ...
                return `${variant.title} ${variant.sku} ${variant.barcode ?? ''}`
                    .toLowerCase()
    ...
    const fetchVariantsByProduct = useCallback(
        async (productId: string, signal?: AbortSignal): Promise<VariantRow[]> => {
    ...
    const createVariantRemote = useCallback(async (): Promise<void> => {
        ...
        try {
            const response = await apiJson<ApiItemResponse<DbVariant>>(
                `/api/admin/commerce/variants`,
                {
                    method: 'POST',
                    body: JSON.stringify(
                        uiToDbCreatePayload({
    ...
    const duplicateVariantRemote = useCallback(
        async (id: string): Promise<void> => {
            ...
            try {
                ...
                modal.success(
                    'Success',
                    `Duplicated “${sourceVariant.title || sourceVariant.sku}” successfully.`,
                );
            } catch (err: unknown) {
        ...
        async (id: string): Promise<void> => {
            ...
            modal.confirmDelete(
                ...
                async () => {
                    ...
                    try {
                        await apiJson<null>(`/api/admin/commerce/variants/${id}`, {
                            method: 'DELETE',
                        });
                        await reloadActiveProductVariants();
                        modal.success(
                            'Success',
                            `Deleted “${variant.title || variant.sku}” successfully.`,
                        );
                    } catch (err: unknown) {
    ...
    const reloadImages = useCallback(async (variantId: string): Promise<void> => {
        const response = await apiJson<ApiListResponse<DbVariantImage>>(
            `/api/admin/commerce/variants/${variantId}/image`,
        );
    ...
    const addImageByUrlRemote = useCallback(
        async (url: string, showSuccess = true): Promise<void> => {
            ...
            try {
                await apiJson<ApiItemResponse<unknown>>(
                    `/api/admin/commerce/variants/${activeVariant.id}/image`,
                    {
                        method: 'POST',
    ...
    const uploadImageFileRemote = useCallback(
        async (file: File): Promise<void> => {
            ...
            try {
                ...
                const uploadResponse = await apiJson<UploadResponse>(
                    `/api/admin/commerce/variants/upload`,
                    {
                        method: 'POST',
                        body: formData,
                    },
    ...
    const publishActiveVariant = useCallback(async (): Promise<void> => {
        ...
        modal.success(
            'Success',
            `Published “${publishForm.title || activeVariant.sku}” successfully.`,
        );
    }, [activeVariant, variantForm, saveVariantRemote, modal]);
                ...
                <aside className={styles.sidebar}>
                                        ...
                                        <div className={styles.productLeft}>
                                            <div className={styles.thumb}>
                                                ...
                                                <img
                                                    ...
                                                    src={
                                                        product.image ||
                                                        'https://picsum.photos/seed/na/200/200'
                                                    }
                                                    alt={product.name}
                                                />
                                            </div>
                    ...
                    <div className={styles.sidebarFooter}>
                        <div className={styles.tip}>
                            ...
                            <span>
                                Variants unique theo{' '}
                                <span className={styles.mono}>[siteId, sku]</span>.
                            </span>
                        </div>
                    </div>
                </aside>
                                            ...
                                            productVariants.map((variant) => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <td>
                                                            <span
                                                                className={`${styles.status} ${variant.isActive ? styles.ok : styles.off}`}
                                                            >
                                                                <i
                                                                    className={`bi ${variant.isActive ? 'bi-check2-circle' : 'bi-pencil'}`}
                                                                />
                                                                {variant.isActive
                                                                    ? 'ACTIVE'
                                                                    : 'DRAFT'}
                                    ...
                                    <div className={styles.panelBody}>
                                        ...
                                        <div className={styles.inputWrap}>
                                            ...
                                            <input
                                                ...
                                                onChange={(event) =>
                                                    patchVariantForm({ title: event.target.value })
                                                }
                                            />
                                        </div>
                                        ...
                                        <div className={styles.inputWrap}>
                                            ...
                                            <input
                                                ...
                                                onChange={(event) =>
                                                    patchVariantForm({
                                                        sku: slugSku(event.target.value),
                                                    })
                                                }
                                            />
                                        </div>
                                            ...
                                            <div>
                                                ...
                                                <div className={styles.inputWrap}>
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={(event) =>
                                                            patchVariantForm({
                                                                weight: normalizeNullableNumber(
                                                                    event.target.value,
                                                                ),
                                                            })
                                                        }
                                            ...
                                            <div>
                                                ...
                                                <div className={styles.inputWrap}>
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={(event) =>
                                                            patchVariantForm({
                                                                length: normalizeNullableNumber(
                                                                    event.target.value,
                                                                ),
                                                            })
                                                        }
                                        ...
                                        <div>
                                            ...
                                            <div className={styles.inputWrap}>
                                                ...
                                                <input
                                                    ...
                                                    onChange={(event) =>
                                                        patchVariantForm({
                                                            height: normalizeNullableNumber(
                                                                event.target.value,
                                                            ),
                                                        })
                                                    }
                                        ...
                                        <div className={styles.images}>
                                            ...
                                            !localPreviewUrl ? (
                                                <div className={styles.emptySmall}>
                                                    No images. Upload image, kéo thả ảnh hoặc thêm
                                                    image URL.
                                                </div>
                                            ) : (
