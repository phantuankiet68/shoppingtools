...
import type { HeaderFashionProps } from "@/components/admin/shared/templates/ShopTemplate/Ui/header/HeaderFashion";
...
const HeaderFashionEditor: React.FC<EditorProps> = ({ props, updateActive }) => {
  ...
  const categoryString =
    props.searchCategoryOptions && props.searchCategoryOptions.length ? props.searchCategoryOptions.join(", ") : "";
    ...
    <>
      ...
      <Section title="Thương hiệu & logo">
        <LabeledRow label="Tên brand">
          <TextInput
            ...
            placeholder="Aurora Wear / Header Fashion..."
          />
        </LabeledRow>
        ...
        <LabeledRow label="Tagline">
          <TextInput
            ...
            onChange={(v) => updateActive({ brandTagline: v })}
            placeholder="Mix & 2026, slogan..."
          />
        </LabeledRow>
      ...
      <Section title="Thanh tìm kiếm">
        <LabeledRow label="Placeholder">
          <TextInput
            ...
            placeholder="Tìm áo, quần, váy, outfit..."
          />
        </LabeledRow>

        <LabeledRow label="Hint nhỏ bên phải">
          <TextInput
            value={props.searchHint ?? ""}
            onChange={(v) => updateActive({ searchHint: v })}
            placeholder="Enter để gợi ý outfit"
          />
        </LabeledRow>

        <LabeledRow label="Thông báo khi không nhập gì">
          <TextInput
            value={props.searchEmptyMessage ?? ""}
            onChange={(v) => updateActive({ searchEmptyMessage: v })}
            placeholder="Hãy nhập sản phẩm hoặc outfit..."
          />
        </LabeledRow>

        <LabeledRow label="Prefix thông báo khi tìm kiếm">
          <TextInput
            value={props.searchAlertPrefix ?? ""}
            onChange={(v) => updateActive({ searchAlertPrefix: v })}
            placeholder='Aurora Wear đang gợi ý outfit cho: "..."'
          />
        </LabeledRow>
        ...
        <LabeledRow label="Danh mục tìm kiếm (ngăn cách bởi dấu phẩy)">
          <TextInput
            ...
            placeholder="Tất cả danh mục, Nữ, Nam, Unisex..."
          />
        </LabeledRow>
      </Section>
      ...
      <Section title="Badge & chế độ hiển thị">
        <LabeledRow label="Hiện nút Yêu thích">
          <Checkbox
            checked={props.showFavorites ?? false}
            onChange={(checked) => updateActive({ showFavorites: checked })}
          />
        </LabeledRow>
        ...
        <LabeledRow label="Số lượng Giỏ hàng">
          <TextInput
            value={typeof props.cartCount === "number" ? String(props.cartCount) : ""}
            onChange={(v) => updateActive({ cartCount: toInt(v, props.cartCount ?? 0) })}
            placeholder="3"
          />
        </LabeledRow>
      ...
      <Section title="Nav / menu tự động">
        ...
        <LabeledRow label="Set key (menu set)">
          <TextInput
            value={props.setKey ?? ""}
            onChange={(v) => updateActive({ setKey: v })}
            placeholder="home, main-nav..."
          />
        </LabeledRow>
        ...
        <LabeledRow label="Site ID (optional)">
          <TextInput
            value={props.siteId ?? ""}
            onChange={(v) => updateActive({ siteId: v })}
            placeholder="site id nếu dùng multi-site..."
          />
        </LabeledRow>

        <LabeledRow label="ID nav chính (primaryNavId)">
          <TextInput
            value={props.primaryNavId ?? ""}
            onChange={(v) => updateActive({ primaryNavId: v })}
            placeholder="id item để tô màu primary..."
          />
        </LabeledRow>
      </Section>
      ...
      <Section title="Mobile & popup search">
        <LabeledRow label="Tiêu đề menu mobile">
          <TextInput
            value={props.mobileMenuTitle ?? ""}
            onChange={(v) => updateActive({ mobileMenuTitle: v })}
        ...
        <LabeledRow label="Bottom nav: Yêu thích">
          <TextInput
            value={props.bottomNavWishlistLabel ?? ""}
            onChange={(v) => updateActive({ bottomNavWishlistLabel: v })}
        ...
        <LabeledRow label="Popup search placeholder (mobile)">
          <TextInput
            value={props.popupSearchPlaceholder ?? ""}
            onChange={(v) => updateActive({ popupSearchPlaceholder: v })}
            placeholder="Tìm áo, quần, váy, outfit..."
          />
        </LabeledRow>
      </Section>
