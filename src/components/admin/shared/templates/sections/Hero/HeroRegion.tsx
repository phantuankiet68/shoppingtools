...
import cls from "@/styles/templates/sections/Hero/HeroRegion.module.css";
...
export type HeroRegionQuickLink = {
  label: string;
  href: string;
  icon?: string;
};

export type HeroRegionFeature = {
  label: string;
  value?: string;
  icon?: string;
  href?: string;
...
export type HeroRegionProps = {
  siteId?: string;
  categoryApiPath?: string;
  categoryBasePath?: string;
  activeOnly?: boolean;
  onlyRootCategories?: boolean;
  slides?: HeroRegionSlide[];
  spots?: HeroRegionSpot[];
  quickLinks?: HeroRegionQuickLink[];
  features?: HeroRegionFeature[];
  autoMs?: number;
  preview?: boolean;
};

/* ================= Defaults ================= */
const DEFAULT_SLIDES: HeroRegionSlide[] = [
  {
    imageSrc: "/assets/images/product.jpg",
    mobileImageSrc: "/assets/images/product.jpg",
    href: "/collections/new",
    alt: "Soft region banner 1",
  ...
  },
  {
    ...
    alt: "Soft region banner 2",
    bg: "linear-gradient(135deg, #fff3ec 0%, #fff8f5 55%, #ffffff 100%)",
  },
  {
    imageSrc: "/assets/images/product.jpg",
    mobileImageSrc: "/assets/images/product.jpg",
    href: "/collections/decor",
    alt: "Soft region banner 3",
    bg: "linear-gradient(135deg, #f6efff 0%, #fcf8ff 55%, #ffffff 100%)",
  },
];

const DEFAULT_SPOTS: HeroRegionSpot[] = [
  {
    title: "Quà xinh mỗi ngày",
    sub: "Gợi ý nhẹ nhàng dành cho người bạn thương",
    href: "/collections/gifts",
    imageSrc: "/assets/images/product.jpg",
    icon: "bi-heart",
    tone: "rose",
  },
  {
    ...
    sub: "Trang trí ấm áp và tinh tế hơn",
    href: "/collections/decor",
    imageSrc: "/assets/images/product.jpg",
    icon: "bi-house-heart",
    tone: "pearl",
  ...
  {
    title: "Beauty được yêu thích",
    sub: "Những món đang được chọn nhiều",
...
const DEFAULT_QUICK_LINKS: HeroRegionQuickLink[] = [
...
const DEFAULT_FEATURES: HeroRegionFeature[] = [
...
function resolveToneClass(tone?: HeroRegionSpot["tone"]): string {
  switch (tone) {
    case "peach":
      return cls.spotTonePeach;
    case "pearl":
      return cls.spotTonePearl;
    case "mint":
      return cls.spotToneMint;
    case "violet":
      return cls.spotToneViolet;
    case "rose":
    default:
      return cls.spotToneRose;
  }
}
...
export function HeroRegion({
  ...
  useEffect(() => {
    ...
    const loadCategories = async () => {
      try {
        ...
        if (!response.ok) {
          console.error("[HeroRegion] load categories failed:", response.status, await response.text());
        ...
        console.error("[HeroRegion] load categories error:", error);
        if (alive) setCategories([]);
      }
    };
  ...
  const spotItems = useMemo(() => spots.slice(0, 3), [spots]);
  const quickItems = useMemo(() => quickLinks.slice(0, 6), [quickLinks]);
  const featureItems = useMemo(() => features.slice(0, 3), [features]);
  ...
  return (
    <section className={cls.hero} aria-label="Hero Region">
        ...
        <div className={cls.contentCol}>
          <div className={cls.featureRow}>
            {featureItems.map((item, index) => {
              const content = (
                <>
                  <span className={cls.featureIcon}>
                    <i className={`bi ${ensureBootstrapIcon(item.icon)}`} />
                  </span>
                  <div className={cls.featureText}>
                    {item.value ? <div className={cls.featureValue}>{item.value}</div> : null}
                    <div className={cls.featureLabel}>{item.label}</div>
                  </div>
                </>
              );
              ...
              return (
                <React.Fragment key={`${item.href || item.label}-${index}`}>
                  {renderNavTarget(item.href || "/", cls.featureCard, content)}
                </React.Fragment>
              );
            })}
          </div>

          <div className={cls.regionGrid}>
            <div className={cls.heroArea}>
                      ...
                      const slideContent = (
                        ...
                        >
                          ...
                          <div className={cls.slideMedia}>
                            <Image
                              src={slide.imageSrc}
                              alt={slide.alt || "Hero region banner"}
                              fill
              ...
              <div className={cls.spotGrid}>
                {spotItems.map((spot, index) => {
                  const content = (
                    <>
                      <div className={cls.spotCopy}>
                        <span className={cls.spotIcon}>
                          <i className={`bi ${ensureBootstrapIcon(spot.icon)}`} />
                        </span>
                        <div className={cls.spotTitle}>{spot.title}</div>
                        {spot.sub ? <div className={cls.spotSub}>{spot.sub}</div> : null}
                      </div>

                      <div className={cls.spotMedia}>
                        {spot.imageSrc ? (
                          <Image
                            src={spot.imageSrc}
                            alt=""
                            ...
                            className={cls.spotImage}
                          />
                        ) : (
                          <div className={cls.spotFallback}>
                            <i className={`bi ${ensureBootstrapIcon(spot.icon)}`} />
                          </div>
                        )}
                      </div>
                  ...
                  return (
                    <React.Fragment key={`${spot.href}-${index}`}>
                      {renderNavTarget(spot.href, `${cls.spotCard} ${resolveToneClass(spot.tone)}`, content)}
                    </React.Fragment>
                  );
                })}
              </div>
            ...
            <aside className={cls.sidePanel} aria-label="Quick region links">
              ...
              <div className={cls.quickPanel}>
                <div className={cls.quickPanelTitle}>Lối tắt vùng chọn</div>
...
/* ================= RegItem ================= */
export const SHOP_HERO_REGION: RegItem = {
  kind: "HeroRegion",
  label: "Hero Region",
  defaults: {
    autoMs: 4200,
    ...
    spots: JSON.stringify(DEFAULT_SPOTS, null, 2),
    quickLinks: JSON.stringify(DEFAULT_QUICK_LINKS, null, 2),
    features: JSON.stringify(DEFAULT_FEATURES, null, 2),
  },
  inspector: [
    ...
    { key: "spots", label: "Spots (JSON)", kind: "textarea", rows: 10 },
    { key: "quickLinks", label: "Quick links (JSON)", kind: "textarea", rows: 10 },
    { key: "features", label: "Features (JSON)", kind: "textarea", rows: 10 },
  ],
  render: (props) => {
    const slides = safeJsonParse<HeroRegionSlide[]>(props.slides);
    const spots = safeJsonParse<HeroRegionSpot[]>(props.spots);
    const quickLinks = safeJsonParse<HeroRegionQuickLink[]>(props.quickLinks);
    const features = safeJsonParse<HeroRegionFeature[]>(props.features);
    ...
    return (
      <div className="sectionContainer" aria-label="Shop Hero Region">
        <HeroRegion
          autoMs={autoMs}
          slides={slides}
          spots={spots}
