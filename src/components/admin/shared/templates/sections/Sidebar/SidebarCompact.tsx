...
import cls from "@/styles/templates/sections/Sidebar/SidebarCompact.module.css";
import type { RegItem } from "@/lib/ui-builder/types";

/* ================= Types ================= */
export type SidebarCompactNavChildItem = {
  label: string;
  href: string;
  icon?: string;
  active?: boolean;
...
export type SidebarCompactNavItem = {
  label: string;
  href?: string;
  icon: string;
  badge?: string;
  active?: boolean;
  children?: SidebarCompactNavChildItem[];
};

export type SidebarCompactProfile = {
  name: string;
  email: string;
  avatarSrc: string;
};

export type SidebarCompactBrand = {
  name: string;
  logoText?: string;
  dotColor?: string;
};

export type SidebarCompactProps = {
  brand?: SidebarCompactBrand;

  searchPlaceholder?: string;

  mainTitle?: string;
  mainItems?: SidebarCompactNavItem[];

  otherTitle?: string;
  otherItems?: SidebarCompactNavItem[];

  lightLabel?: string;
  darkLabel?: string;

  profile?: SidebarCompactProfile;

  preview?: boolean;
};

/* ================= Defaults ================= */
const DEFAULT_BRAND: SidebarCompactBrand = {
  name: "Shopiva",
  logoText: "S",
  dotColor: "#7c3aed",
};

const DEFAULT_MAIN_ITEMS: SidebarCompactNavItem[] = [
  { label: "Overview", href: "/dashboard", icon: "bi-grid-1x2-fill", active: true },
  { label: "Orders", href: "/orders", icon: "bi-bag-check-fill", badge: "12" },
  {
    label: "Products",
    href: "/products",
    ...
    children: [
      ...
      { label: "Collections", href: "/products/collections", icon: "bi-stars", caption: "Themes" },
      { label: "Inventory", href: "/products/inventory", icon: "bi-boxes", caption: "Stock" },
    ],
  },
...
const DEFAULT_OTHER_ITEMS: SidebarCompactNavItem[] = [
...
const DEFAULT_PROFILE: SidebarCompactProfile = {
...
export function SidebarCompact({
  ...
  const renderLeafLink = (item: SidebarCompactNavItem, className: string, labelClass: string, iconClass?: string) => {
    const content = (
      <>
  ...
  const renderChildLink = (item: SidebarCompactNavChildItem, idx: number) => {
    const content = (
      <>
          ...
          <div className={cls.brandWrap}>
            ...
            {!collapsed ? (
              <div className={cls.brandText}>
                ...
                <div className={cls.brandMeta}>Commerce UI</div>
              </div>
            ) : null}
          </div>
        ...
        {!collapsed ? (
          <div className={cls.miniCard}>
            <div className={cls.miniCardTop}>
              <span className={cls.miniPill}>Pro Store</span>
              <span className={cls.miniTrend}>+16%</span>
            </div>

            <div className={cls.miniTitle}>Compact sales workspace built for modern storefronts.</div>

            <div className={cls.miniStats}>
              <div className={cls.miniStat}>
                <span className={cls.miniValue}>842</span>
                <span className={cls.miniLabel}>Orders</span>
              </div>
              <div className={cls.miniStat}>
                <span className={cls.miniValue}>$18.4k</span>
                <span className={cls.miniLabel}>Revenue</span>
              </div>
            </div>
          </div>
        ) : null}
...
/* ================= RegItem ================= */
export const SHOP_SIDEBAR_COMPACT: RegItem = {
  kind: "SidebarCompact",
  ...
  render: (p) => {
    const brand = safeJson<SidebarCompactBrand>(p.brand);
    const mainItems = safeJson<SidebarCompactNavItem[]>(p.mainItems);
    const otherItems = safeJson<SidebarCompactNavItem[]>(p.otherItems);
    const profile = safeJson<SidebarCompactProfile>(p.profile);

    return (
      <div className="sectionContainer" aria-label="Shop Sidebar Compact">
        <SidebarCompact
          brand={brand}
          searchPlaceholder={String(p.searchPlaceholder || "Search store...")}
          mainTitle={String(p.mainTitle || "Main")}
          mainItems={mainItems}
