...
import cls from "@/styles/templates/sections/Sidebar/SidebarTicker.module.css";
import type { RegItem } from "@/lib/ui-builder/types";

/* ================= Types ================= */
export type SidebarTickerNavChildItem = {
  label: string;
  href: string;
  icon?: string;
  active?: boolean;
...
export type SidebarTickerNavItem = {
  label: string;
  href?: string;
  icon: string;
  badge?: string;
  active?: boolean;
  children?: SidebarTickerNavChildItem[];
};

export type SidebarTickerProfile = {
  name: string;
  email: string;
  avatarSrc: string;
};

export type SidebarTickerBrand = {
  name: string;
  logoText?: string;
  dotColor?: string;
};

export type SidebarTickerStatItem = {
  label: string;
  value: string;
  trend?: string;
};

export type SidebarTickerProps = {
  brand?: SidebarTickerBrand;

  searchPlaceholder?: string;

  mainTitle?: string;
  mainItems?: SidebarTickerNavItem[];

  otherTitle?: string;
  otherItems?: SidebarTickerNavItem[];

  tickerTitle?: string;
  tickerItems?: SidebarTickerStatItem[];

  lightLabel?: string;
  darkLabel?: string;

  profile?: SidebarTickerProfile;

  preview?: boolean;
};

/* ================= Defaults ================= */
const DEFAULT_BRAND: SidebarTickerBrand = {
  name: "Flowmerce",
  logoText: "F",
  dotColor: "#7c3aed",
};

const DEFAULT_MAIN_ITEMS: SidebarTickerNavItem[] = [
  { label: "Overview", href: "/dashboard", icon: "bi-grid-1x2-fill", active: true },
  { label: "Orders", href: "/orders", icon: "bi-bag-check-fill", badge: "36" },
  {
    label: "Products",
    href: "/products",
    icon: "bi-box-seam-fill",
    children: [
      { label: "All Products", href: "/products/all", icon: "bi-grid", active: true, caption: "Live catalog" },
      { label: "Collections", href: "/products/collections", icon: "bi-stars", caption: "Season themes" },
      { label: "Inventory", href: "/products/inventory", icon: "bi-boxes", caption: "Stock pulse" },
    ],
  },
...
const DEFAULT_OTHER_ITEMS: SidebarTickerNavItem[] = [
...
const DEFAULT_TICKER_ITEMS: SidebarTickerStatItem[] = [
  { label: "GMV", value: "$128.4k", trend: "+14%" },
  { label: "Orders", value: "1,248", trend: "+9%" },
  { label: "AOV", value: "$62", trend: "+4%" },
];

const DEFAULT_PROFILE: SidebarTickerProfile = {
...
export function SidebarTicker({
  ...
  const ticker = useMemo(() => tickerItems ?? DEFAULT_TICKER_ITEMS, [tickerItems]);
  ...
  const renderLeafLink = (item: SidebarTickerNavItem, className: string, labelClass: string, iconClass?: string) => {
    const content = (
      <>
  ...
  const renderChildLink = (item: SidebarTickerNavChildItem, idx: number) => {
    const content = (
      <>
        ...
        <div className={cls.topBar}>
          <div className={cls.brandWrap}>
            <div className={cls.brandMark}>
              <span className={cls.brandGlyph}>{bd.logoText || "F"}</span>
            </div>

            {!collapsed ? (
              <div className={cls.brandText}>
                <div className={cls.brandName}>{bd.name}</div>
                <div className={cls.brandMeta}>Commerce Ticker</div>
              </div>
            ) : null}
          </div>
        ...
        {!collapsed ? (
          <div className={cls.heroCard}>
            <div className={cls.heroHead}>
              ...
              <span className={cls.heroStatus}>Live</span>
            </div>

            <div className={cls.heroTitle}>High-clarity sales pulse for premium storefront teams.</div>
            <div className={cls.heroText}>
              A 2026-style ecommerce sidebar with ticker metrics, stronger hierarchy and faster operational flow.
            </div>

            <div className={cls.tickerWrap} aria-label="Sales ticker">
              <div className={cls.tickerTrack}>
                {[...ticker, ...ticker].map((item, index) => (
                  <div key={index} className={cls.tickerItem}>
                    <span className={cls.tickerLabel}>{item.label}</span>
                    <span className={cls.tickerValue}>{item.value}</span>
                    {item.trend ? <span className={cls.tickerTrend}>{item.trend}</span> : null}
                  </div>
                ))}
              </div>
            </div>
...
/* ================= RegItem ================= */
export const SHOP_SIDEBAR_TICKER: RegItem = {
  kind: "SidebarTicker",
  ...
  inspector: [
    ...
    { key: "tickerTitle", label: "Ticker title", kind: "text" },
  ...
  render: (p) => {
    const brand = safeJson<SidebarTickerBrand>(p.brand);
    const mainItems = safeJson<SidebarTickerNavItem[]>(p.mainItems);
    const otherItems = safeJson<SidebarTickerNavItem[]>(p.otherItems);
    const tickerItems = safeJson<SidebarTickerStatItem[]>(p.tickerItems);
    const profile = safeJson<SidebarTickerProfile>(p.profile);

    return (
      <div className="sectionContainer" aria-label="Shop Sidebar Ticker">
        <SidebarTicker
          brand={brand}
          ...
          tickerTitle={String(p.tickerTitle || "Live Sales Ticker")}
