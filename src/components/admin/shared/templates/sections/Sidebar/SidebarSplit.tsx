...
import cls from "@/styles/templates/sections/Sidebar/SidebarSplit.module.css";
import type { RegItem } from "@/lib/ui-builder/types";

/* ================= Types ================= */
export type SidebarSplitNavChildItem = {
  label: string;
  href: string;
  icon?: string;
  active?: boolean;
...
export type SidebarSplitNavItem = {
  label: string;
  href?: string;
  icon: string;
  badge?: string;
  active?: boolean;
  children?: SidebarSplitNavChildItem[];
};

export type SidebarSplitProfile = {
  name: string;
  email: string;
  avatarSrc: string;
};

export type SidebarSplitBrand = {
  name: string;
  logoText?: string;
  dotColor?: string;
};

export type SidebarSplitStatItem = {
  label: string;
  value: string;
};

export type SidebarSplitProps = {
  brand?: SidebarSplitBrand;

  searchPlaceholder?: string;

  mainTitle?: string;
  mainItems?: SidebarSplitNavItem[];

  otherTitle?: string;
  otherItems?: SidebarSplitNavItem[];

  spotlightTitle?: string;
  spotlightText?: string;
  spotlightStats?: SidebarSplitStatItem[];

  lightLabel?: string;
  darkLabel?: string;

  profile?: SidebarSplitProfile;

  preview?: boolean;
};

/* ================= Defaults ================= */
const DEFAULT_BRAND: SidebarSplitBrand = {
  name: "Sellframe",
  logoText: "S",
  dotColor: "#5b8def",
};

const DEFAULT_MAIN_ITEMS: SidebarSplitNavItem[] = [
...
const DEFAULT_OTHER_ITEMS: SidebarSplitNavItem[] = [
...
const DEFAULT_SPOTLIGHT_STATS: SidebarSplitStatItem[] = [
  { label: "Revenue", value: "$42.8k" },
  { label: "Orders", value: "1,284" },
];

const DEFAULT_PROFILE: SidebarSplitProfile = {
...
export function SidebarSplit({
  brand,
  searchPlaceholder = "Search products, orders, stores...",
  mainTitle = "Commerce",
  mainItems,
  otherTitle = "Workspace",
  otherItems,
  spotlightTitle = "Split layout for modern sales teams",
  spotlightText = "A premium 2026 ecommerce sidebar with clearer hierarchy, stronger visual rhythm and better focus for daily operations.",
  spotlightStats,
  lightLabel = "Light",
  darkLabel = "Dark",
  profile,
  ...
  const stats = useMemo(() => spotlightStats ?? DEFAULT_SPOTLIGHT_STATS, [spotlightStats]);
  ...
  const renderLeafLink = (item: SidebarSplitNavItem, className: string, labelClass: string, iconClass?: string) => {
    const content = (
      <>
  ...
  const renderChildLink = (item: SidebarSplitNavChildItem, idx: number) => {
    const content = (
      <>
  ...
  return (
    <aside
      ...
      aria-label="Sidebar split"
      style={{ ["--brand-dot" as string]: bd.dotColor || "#5b8def" }}
    >
      <div className={cls.inner}>
          ...
          <div className={cls.brandWrap}>
            ...
            {!collapsed ? (
              <div className={cls.brandText}>
                ...
                <div className={cls.brandMeta}>Sales OS</div>
              </div>
            ) : null}
          </div>
        ...
        {!collapsed ? (
          <div className={cls.spotlightCard}>
            <div className={cls.spotlightBadge}>
              <i className="bi bi-stars" aria-hidden="true" />
              <span>Header Mode 2026</span>
            </div>

            <div className={cls.spotlightTitle}>{spotlightTitle}</div>
            <div className={cls.spotlightText}>{spotlightText}</div>

            <div className={cls.spotlightStats}>
              {stats.map((item, index) => (
                <div key={index} className={cls.spotlightStat}>
                  <span className={cls.spotlightValue}>{item.value}</span>
                  <span className={cls.spotlightLabel}>{item.label}</span>
                </div>
              ))}
            </div>
          </div>
...
/* ================= RegItem ================= */
export const SHOP_SIDEBAR_SPLIT: RegItem = {
  kind: "SidebarSplit",
  label: "Sidebar Split",
  defaults: {
    ...
    spotlightTitle: "Split layout for modern sales teams",
    spotlightText:
      "A premium 2026 ecommerce sidebar with clearer hierarchy, stronger visual rhythm and better focus for daily operations.",
    spotlightStats: JSON.stringify(DEFAULT_SPOTLIGHT_STATS, null, 2),
    lightLabel: "Light",
    darkLabel: "Dark",
  ...
  inspector: [
    ...
    { key: "spotlightText", label: "Spotlight text", kind: "textarea", rows: 5 },
    { key: "spotlightStats", label: "Spotlight stats (JSON)", kind: "textarea", rows: 8 },
  ...
  render: (p) => {
    const brand = safeJson<SidebarSplitBrand>(p.brand);
    const mainItems = safeJson<SidebarSplitNavItem[]>(p.mainItems);
    const otherItems = safeJson<SidebarSplitNavItem[]>(p.otherItems);
    const spotlightStats = safeJson<SidebarSplitStatItem[]>(p.spotlightStats);
    const profile = safeJson<SidebarSplitProfile>(p.profile);

    return (
      <div className="sectionContainer" aria-label="Shop Sidebar Split">
        <SidebarSplit
          brand={brand}
          searchPlaceholder={String(p.searchPlaceholder || "Search products, orders, stores...")}
          mainTitle={String(p.mainTitle || "Commerce")}
          mainItems={mainItems}
          otherTitle={String(p.otherTitle || "Workspace")}
          otherItems={otherItems}
          spotlightTitle={String(p.spotlightTitle || "Split layout for modern sales teams")}
          spotlightText={String(
            p.spotlightText ||
              "A premium 2026 ecommerce sidebar with clearer hierarchy, stronger visual rhythm and better focus for daily operations.",
          )}
