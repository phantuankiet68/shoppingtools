...
import cls from "@/styles/templates/sections/Header/HeaderRegion.module.css";
...
const DEFAULT_CATEGORY_ITEMS: CategoryMenuItem[] = [
  { label: "Mỹ phẩm", href: "/cosmetics", emoji: "💄" },
  { label: "Chăm sóc da", href: "/skincare", emoji: "✨" },
  { label: "Chăm sóc tóc", href: "/haircare", emoji: "🧴" },
  { label: "Nước hoa", href: "/perfume", emoji: "🌸" },
  { label: "Mẹ & bé", href: "/mom-baby", emoji: "🍼" },
  { label: "Thiết bị làm đẹp", href: "/beauty-tools", emoji: "🪞" },
  { label: "Sức khỏe", href: "/health", emoji: "💊" },
...
const DEFAULT_NOTIFICATIONS: ProductNotificationItem[] = [
  {
    id: "1",
    title: "Khu vực của bạn có ưu đãi mới",
    message: "Flash sale độc quyền cho khách hàng nội thành hôm nay.",
    time: "15m",
    href: "/sale",
    unread: true,
  ...
  },
  {
    ...
    message: "Đơn hàng của bạn đang được giao đến khu vực đã chọn.",
    time: "1h",
    href: "/account/orders",
    unread: true,
    thumbnail: "/assets/images/logo.jpg",
  ...
  {
    id: "3",
    title: "Sản phẩm vừa về hàng",
    message: "Một số sản phẩm nổi bật đã được bổ sung lại kho.",
    time: "3h",
    href: "/new-arrivals",
    unread: false,
    thumbnail: "/assets/images/logo.jpg",
...
const DEFAULT_CART_ITEMS: CartPopupItem[] = [
  {
    id: "1",
    name: "Laneige Lip Sleeping Mask Mini",
    price: 129000,
    qty: 1,
...
export function HeaderRegion({
  ...
  brandSub = "Regional Commerce",
  logoSrc,
  logoAlt = "",
  searchPlaceholder = "Tìm sản phẩm, thương hiệu, khu vực ưu đãi...",
  onSearchSubmit,
  badgeStoreLocator = 2,
  badgeCart = 0,
  navItems,
  ...
  regionLabel = "Khu vực giao hàng",
  regionValue = "Hồ Chí Minh",
  hotlineText = "Hotline: 1900 9999",
  promoText = "Freeship nội thành cho đơn từ 299K",
}: HeaderRegionProps) {
  ...
  const openMegaMenu = (idx: number) => {
    clearMegaCloseTimer();
    setOpenMegaIndex(idx);
  };
    ...
    <header className={cls.header} ref={rootRef}>
      <div className={cls.mainBar}>
        <div className={cls.mainInner}>
            ...
            {preview ? (
              <a className={cls.brand} href="#" aria-label={brandName} onClick={onBlockClick}>
                ...
                <span className={cls.brandText}>
                  ...
                  <small className={cls.brandSub}>{brandSub}</small>
                </span>
              </a>
            ) : (
          ...
          <div className={cls.regionCol}>
            <button
              type="button"
              className={cls.regionButton}
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
              }}
            >
              ...
              <span className={cls.regionText}>
                <small>{regionLabel}</small>
                <strong>{regionValue}</strong>
              </span>

              <i className={`bi bi-chevron-down ${cls.regionCaret}`} />
            </button>
          </div>
          ...
          <div className={cls.actionCol}>
            ...
            >
              ...
              {badgeCart > 0 || cartItems.length > 0 ? (
                <span className={cls.iconBadge}>{badgeCart || cartItems.length}</span>
              ) : null}
            </button>
            ...
            >
              ...
              <span className={cls.accountMeta}>
                <strong>{isLoggedIn ? displayName : authChecked ? "Đăng nhập" : "Loading..."}</strong>
            ...
            {notificationOpen && (
              ...
              >
                <div className={cls.dropdownHead}>
                  <div>
                    <strong>Thông báo khu vực</strong>
                    <small>{unreadNotificationCount} nội dung chưa đọc</small>
                  </div>
                </div>
            ...
            {cartOpen && (
              ...
              >
                ...
                <div className={cls.dropdownFootGrid}>
                  {preview ? (
            ...
            {isLoggedIn && accountOpen ? (
              ...
              >
                ...
                <button type="button" className={cls.logoutButton} onClick={handleLogout}>
      ...
      <div className={`${cls.navWrap} ${mobileNavOpen ? cls.navWrapOpen : ""}`}>
        <div className={cls.navInner}>
          <div className={cls.navLeft}>
            <div
              ...
              onMouseEnter={() => {
                if (!preview) openCategoryMenu();
              }}
              onMouseLeave={() => {
                if (!preview) closeCategoryMenu();
              }}
              onClick={(e) => {
                if (preview) {
                  e.preventDefault();
            ...
            >
              ...
              <div
                className={cls.categoryDropdown}
                onMouseEnter={() => {
                  if (!preview) openCategoryMenu();
                }}
                onMouseLeave={() => {
              ...
              items.map((it, idx) => {
                ...
                return (
                  <div
                    ...
                    onMouseEnter={() => {
                      if (!preview) openMegaMenu(idx);
                    }}
                    onMouseLeave={() => {
                      if (!preview) closeMegaMenu();
                    }}
                    onClick={(e) => {
                      if (preview) {
                        e.preventDefault();
          ...
          <div className={cls.navRight}>
            {preview ? (
              <a href="#" className={cls.campaignPill} onClick={onBlockClick}>
                <i className="bi bi-lightning-charge-fill" />
                <span>Ưu đãi theo khu vực</span>
              </a>
            ) : (
              <Link href={"/sale" as Route} className={cls.campaignPill}>
                <i className="bi bi-lightning-charge-fill" />
                <span>Ưu đãi theo khu vực</span>
              </Link>
            )}
...
export const SHOP_HEADER_REGION: RegItem = {
  kind: "HeaderRegion",
  label: "Header Region",
  defaults: {
    brandHref: "/",
  ...
  inspector: [
    ...
    { key: "regionValue", label: "Region Value", kind: "text" },
    { key: "hotlineText", label: "Hotline Text", kind: "text" },
    { key: "promoText", label: "Promo Text", kind: "text" },
  ...
  render: (props) => {
    ...
    return (
      <div aria-label="Shop Header Region">
        <HeaderRegion
          ...
          regionLabel={p.regionLabel || "Khu vực giao hàng"}
          regionValue={p.regionValue || "Hồ Chí Minh"}
          hotlineText={p.hotlineText || "Hotline: 1900 9999"}
          promoText={p.promoText || "Freeship nội thành cho đơn từ 299K"}
        />
      </div>
    );
  },
