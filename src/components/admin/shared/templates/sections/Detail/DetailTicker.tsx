...
function getImageFromRecord(item: UnknownRecord): string {
    const imageObject =
        item.image && typeof item.image === 'object' ? (item.image as UnknownRecord) : undefined;
    const thumbnailObject =
        item.thumbnail && typeof item.thumbnail === 'object'
            ? (item.thumbnail as UnknownRecord)
            : undefined;
    const coverImageObject =
        item.coverImage && typeof item.coverImage === 'object'
            ? (item.coverImage as UnknownRecord)
            : undefined;
...
function computeReviewCount(item: UnknownRecord): number | undefined {
    const count = toNumber(
        item.reviewCount ?? item.reviewsCount ?? item.totalReviews ?? item.numReviews,
        NaN,
    );
...
function detectBrand(item: UnknownRecord, name: string): string | undefined {
    const brandObject =
        item.brand && typeof item.brand === 'object' ? (item.brand as UnknownRecord) : undefined;
...
function detectCategory(item: UnknownRecord): string | undefined {
    ...
    return (
        ...
        toStringSafe(categoryObject?.name) ||
        toStringSafe(item.collection) ||
        undefined
    );
}
...
function normalizeReview(raw: unknown, index: number): ProductReviewItem | null {
    ...
    return {
        ...
        author:
            toStringSafe(item.author) ||
            toStringSafe(item.userName) ||
...
function normalizeProduct(raw: unknown): DetailTickerProduct | null {
    ...
    return {
        id: isProductIdentifier(item.id)
            ? item.id
            : isProductIdentifier(item._id)
              ? item._id
              : undefined,
        name,
        ...
        soldText:
            ...
            (Number.isFinite(soldCount) && soldCount > 0
                ? `${formatCompactCount(soldCount)} sold`
                : '10k+ sold'),
...
function getSavings(product?: DetailTickerProduct | null): number | undefined {
    ...
    if (typeof product.price !== 'number' || typeof product.originalPrice !== 'number')
        return undefined;
    ...
    return (
        <span className={cls.starRow} aria-label={`Rated ${rating ?? 0} out of 5`}>
            {stars.map((state, index) => (
                <span
                    key={`${state}-${index}`}
                    className={state === 'full' ? cls.starFull : cls.starEmpty}
                >
                    ★
                </span>
            ))}
...
export function DetailTicker({
    ...
    const gallery = useMemo(() => {
        ...
        return Array.from(
            new Set([currentProduct.imageSrc, ...(currentProduct.gallery || [])].filter(Boolean)),
        );
    }, [currentProduct]);
    ...
    const detailHref =
        currentProduct?.href ||
        (slugSegments.length > 0 ? `/product-detail/${slugSegments.join('/')}` : '/product-detail');
    ...
    const subtotal =
        typeof currentProduct?.price === 'number' ? currentProduct.price * quantity : undefined;
    ...
    useEffect(() => {
        ...
        async function run() {
            try {
                setLoading(true);
                const normalized = await fetchProductByCandidates(
                    requestCandidates,
                    controller.signal,
                );
    ...
    useEffect(() => {
        ...
        async function run() {
            try {
                ...
                setRemoteReviews(
                    rawList.map(normalizeReview).filter(Boolean) as ProductReviewItem[],
                );
            } catch (error) {
                if (controller.signal.aborted) return;
                ...
                <header className={cls.header}>
                    ...
                    <div className={cls.headerMain}>
                        ...
                        <div className={cls.headerMetrics}>
                            ...
                            <MetricChip
                                label="Reviews"
                                value={formatCompactCount(visibleReviewCount)}
                            />
                            <MetricChip
                                label="Sold"
                                value={currentProduct.soldText || '10k+ sold'}
                            />
                        </div>
                    </div>
                </header>
                    ...
                    <article className={cls.galleryCard} aria-label="Product gallery">
                        <div className={cls.galleryTop}>
                            <div className={cls.badgeGroup}>
                                {currentProduct.badge ? (
                                    <span className={cls.badgePrimary}>{currentProduct.badge}</span>
                                ) : null}
                                {currentProduct.discountPercent ? (
                                    <span className={cls.badgeSecondary}>
                        ...
                        <div className={cls.trustGrid}>
                            <TrustCard
                                ...
                                text="Checked before shipping."
                            />
                            <TrustCard
                                icon="⚡"
                                title="Fast delivery"
                                text={currentProduct.shippingNote || 'Quick dispatch'}
                            />
                            <TrustCard
                                icon="↺"
                                title="Easy returns"
                                text="Support for exchanges and returns."
                            />
                        </div>
                    </article>
                    ...
                    <aside className={cls.purchaseCard} aria-label="Purchase panel">
                        <div className={cls.purchaseTop}>
                            <div className={cls.categoryRow}>
                                <span className={cls.categoryChip}>
                                    {currentProduct.category || 'Fashion'}
                                </span>
                                {currentProduct.stockText ? (
                                    <span className={cls.stockChip}>
                                        {currentProduct.stockText}
                                    </span>
                                ) : null}
                            </div>
                            ...
                            <div className={cls.pricePanel}>
                                <div className={cls.priceMain}>
                                    <strong className={cls.currentPrice}>
                                ...
                                <div className={cls.priceSupport}>
                                    {(savings || currentProduct.savingPrice) && (
                                        <span className={cls.savingsPill}>
                                            Save{' '}
                                            {formatPrice(currentProduct.savingPrice ?? savings)}
                                        </span>
                                    )}
                        ...
                        <div className={cls.summaryGrid}>
                            <div className={cls.summaryItem}>
                                ...
                                <strong className={cls.summaryValue}>
                                    {currentProduct.brand || 'Official seller'}
                                </strong>
                            </div>
                            <div className={cls.summaryItem}>
                        ...
                        <div className={cls.merchantCard}>
                            <div className={cls.merchantTop}>
                                <div className={cls.merchantIdentity}>
                                    <div className={cls.merchantAvatar}>
                                        {(currentProduct.brand || 'S').slice(0, 1).toUpperCase()}
                                    </div>
                                    <div>
                                        <strong className={cls.merchantName}>
                                            {currentProduct.brand || 'Official seller'}
                                        </strong>
                                        <p className={cls.merchantMeta}>
                                ...
                                <MetricChip
                                    ...
                                    value={
                                        currentProduct.stockCount
                                            ? formatCompactCount(currentProduct.stockCount)
                                            : '864'
                                    }
                                />
                            </div>
                    ...
                    <div className={cls.tabPanel}>
                            ...
                            <div className={cls.panelGrid}>
                                <div className={cls.contentCard}>
                                    ...
                                    <h4 className={cls.cardTitle}>
                                        Why this layout performs better
                                    </h4>
                        ...
                        {activeTab === 'reviews' ? (
                            <div className={cls.reviewGrid}>
                                <aside
                                    className={cls.reviewSummaryCard}
                                    aria-label="Rating summary"
                                >
                                    <div className={cls.reviewSummaryTop}>
                                        <strong className={cls.reviewScore}>
                                            {averageRating.toFixed(1)}
                                        </strong>
                                        <ReviewStars rating={averageRating} />
                                        <p className={cls.reviewMetaText}>
                                            Based on {formatCompactCount(visibleReviewCount)}{' '}
                                            reviews • {currentProduct.soldText || '10k+ sold'}
                                        </p>
                                    </div>
                                    ...
                                    <div className={cls.reviewBars}>
                                        {ratingBreakdown.map((item) => (
                                            <div key={item.star} className={cls.reviewBarRow}>
                                                <span className={cls.reviewBarLabel}>
                                                    {item.star} stars
                                                </span>
                                                <div
                                                    className={cls.reviewBarTrack}
                                                    aria-hidden="true"
                                                >
                                                    <div
                                                        className={cls.reviewBarFill}
                                                        style={{ width: `${item.percent}%` }}
                                                    />
                                                </div>
                                                <span className={cls.reviewBarPercent}>
                                                    {item.percent}%
                                                </span>
                                            </div>
                                        ))}
                                                ...
                                                >
                                                    <div className={cls.reviewAvatar}>
                                                        {review.avatar ? (
                                                            ...
                                                            <span
                                                                className={cls.reviewAvatarFallback}
                                                            >
                                                    ...
                                                    <div className={cls.reviewBody}>
                                                        <strong className={cls.reviewAuthor}>
                                                        ...
                                                        <div className={cls.reviewMeta}>
                                                            ...
                                                            {review.verified ? (
                                                                <span className={cls.verifiedBadge}>
                                                                    Verified purchase
                                                                </span>
                                                            ) : null}
                                                        </div>
                                                        {review.title ? (
                                                            <h4 className={cls.reviewTitle}>
                                                                {review.title}
                                                            </h4>
                                                        ) : null}
...
export const SHOP_DETAIL_TICKER: RegItem = {
    ...
    render: (p) => {
        ...
        const productOverride = parsedProduct
            ? normalizeProduct(parsedProduct) || undefined
            : undefined;
        ...
        return (
            <div className="sectionContainer" aria-label="Shop Detail Ticker">
                <DetailTicker
                    ...
                    fallbackBreadcrumbText={String(
                        p.fallbackBreadcrumbText || DEFAULT_FALLBACK_BREADCRUMB,
                    )}
