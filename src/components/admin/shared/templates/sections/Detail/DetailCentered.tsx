...
import cls from "@/styles/templates/sections/Detail/DetailCentered.module.css";
...
export type DetailCenteredProduct = {
...
export type DetailCenteredProps = {
  title?: string;
  eyebrow?: string;
  noteText?: string;
  ...
  preview?: boolean;
  product?: DetailCenteredProduct | null;
...
const DEFAULT_EYEBROW = "Featured product";
const DEFAULT_PRIMARY_CTA = "Buy now";
const DEFAULT_SECONDARY_CTA = "Add to order";
...
function normalizeReview(raw: unknown, index: number): ProductReviewItem | null {
  ...
  return {
    ...
    content:
      ...
      "Matches the description, delivered well, and feels reliable in daily use.",
...
function normalizeProduct(raw: unknown): DetailCenteredProduct | null {
  if (!raw || typeof raw !== "object") return null;
  ...
  return {
    ...
    badge: toStringSafe(item.badge) || (Boolean(item.isNew) ? "New" : "Official store"),
...
function extractProductFromResponse(data: unknown): DetailCenteredProduct | null {
  if (!data) return null;
 ...
 * SEO + derived values
 * ========================= */
function getSavings(product?: DetailCenteredProduct | null): number | undefined {
...
function buildProductJsonLd(product: DetailCenteredProduct) {
  return {
...
function buildAddToOrderPayload(product: DetailCenteredProduct, qty: number) {
  const unitPrice = typeof product.price === "number" ? product.price : 0;
  const subtotal = unitPrice * qty;

  return {
    qty,
    source: "detail-centered",
...
async function fetchProductByCandidates(candidates: string[], signal: AbortSignal): Promise<DetailCenteredProduct | null> {
  for (const url of candidates) {
    try {
      ...
      if (!res.ok) {
        console.warn("DetailCentered fetch product failed:", res.status, url);
        continue;
      }
      ...
      console.warn("DetailCentered fetch candidate error:", url, error);
    }
  }
...
function InfoPill({ label, value }: { label: string; value: string }) {
  return (
    <div className={cls.infoPill}>
      <span className={cls.infoPillLabel}>{label}</span>
      <strong className={cls.infoPillValue}>{value}</strong>
    </div>
  );
}

function TrustItem({ title, text, icon }: { title: string; text: string; icon: string }) {
  return (
 ...
 * Component
 * ========================= */
export function DetailCentered({
  title = DEFAULT_TITLE,
  ...
  const [remoteProduct, setRemoteProduct] = useState<DetailCenteredProduct | null>(null);
  const [remoteReviews, setRemoteReviews] = useState<ProductReviewItem[]>([]);
  const [selectedImage, setSelectedImage] = useState(0);
  const [quantity, setQuantity] = useState(1);

  const headingId = `${regionId}-detail-centered-heading`;
  const reviewHeadingId = `${regionId}-detail-centered-review-heading`;
  const detailsHeadingId = `${regionId}-detail-centered-details-heading`;
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        console.error("DetailCentered fetch error:", error);
    ...
    async function run() {
      try {
       ...
       const reviewUrl = buildReviewsApiUrl(reviewApiPath, productId as ProductIdentifier);
        ...
        if (controller.signal.aborted) return;
        console.error("DetailCentered reviews fetch error:", error);
        setRemoteReviews([]);
      } finally {
        ...
        <header className={cls.header}>
          ...
          <div className={cls.heroIntro}>
            ...
            <div className={cls.heroMeta}>
              <InfoPill label="Rating" value={`${averageRating.toFixed(1)}/5`} />
              <InfoPill label="Reviews" value={formatCompactCount(visibleReviewCount)} />
              <InfoPill label="Sold" value={currentProduct.soldText || "10k+ sold"} />
            </div>
          </div>
        </header>
        ...
        <div className={cls.layout}>
          <article className={cls.galleryCard} aria-label="Product gallery">
            ...
            <div className={cls.trustGrid}>
              <TrustItem title="Verified quality" text="Carefully checked before shipping." icon="✓" />
              <TrustItem
                title="Fast delivery"
                text={currentProduct.shippingNote || "Delivery in 2-4 business days"}
                icon="⚡"
              />
              <TrustItem title="Easy returns" text="Support available if you need to exchange or return." icon="↺" />
            </div>
          </article>
          ...
          <article className={cls.purchaseCard} aria-label="Purchase panel">
            <div className={cls.purchaseTop}>
              ...
              <p className={cls.shortDescription}>
                {currentProduct.shortDescription ||
                  "A refined product page focused on confidence, clarity, and faster decision-making across desktop and mobile."}
              </p>
            </div>
            ...
            <div className={cls.pricePanel}>
              ...
              <div className={cls.priceSupportRow}>
                {(savings || currentProduct.savingPrice) && (
                  <span className={cls.savingsTag}>You save {formatPrice(currentProduct.savingPrice ?? savings)}</span>
                )}
            ...
            <div className={cls.featureList}>
              <div className={cls.featureRow}>
                <span className={cls.featureLabel}>SKU</span>
                <div className={cls.featureValueBlock}>
                  <strong>{currentProduct.sku || currentProduct.tag || "Updating"}</strong>
                  <span>{currentProduct.brand || "Official seller"}</span>
                </div>
              </div>

              <div className={cls.featureRow}>
                <span className={cls.featureLabel}>Offers</span>
                <div className={cls.offerList}>
                  <span className={cls.offerChip}>Instant discount</span>
                  <span className={cls.offerChip}>Store voucher</span>
                  <span className={cls.offerChip}>Shipping support</span>
                  <span className={cls.offerChip}>Priority packaging</span>
                </div>
              </div>
            </div>

            <div className={cls.checkoutPanel}>
            ...
            <div className={cls.merchantCard}>
              <div className={cls.merchantHeader}>
                <div className={cls.merchantIdentity}>
                  ...
                  <div>
                    ...
                    <p className={cls.merchantStatus}>Typically responds within minutes</p>
                  </div>
                </div>
              ...
              <div className={cls.merchantStats}>
                <InfoPill label="Reviews" value={formatCompactCount(visibleReviewCount)} />
                <InfoPill label="Response" value="93%" />
                <InfoPill label="Member" value="6 years" />
                <InfoPill
                  label="Products"
        ...
        <div className={cls.detailGrid}>
          <section className={cls.contentCard} aria-labelledby={detailsHeadingId}>
            ...
            <div className={cls.specGrid}>
              <div className={cls.specItem}>
                ...
                <strong className={cls.specValue}>{currentProduct.category || "Electronics"}</strong>
              </div>
              <div className={cls.specItem}>
              ...
              <div className={cls.specItem}>
                ...
                <strong className={cls.specValue}>{currentProduct.brand || "No brand"}</strong>
              </div>
              <div className={cls.specItem}>
                <span className={cls.specLabel}>Code</span>
                <strong className={cls.specValue}>{currentProduct.sku || currentProduct.tag || "Updating"}</strong>
              </div>
            </div>
          </section>
          ...
          <section className={cls.contentCard} aria-label="Product description">
            ...
            <div className={cls.descriptionBlock}>
              <p>
                {currentProduct.shortDescription ||
                  "This refreshed layout is designed to feel more premium, easier to scan, and stronger for conversion compared with typical template-heavy ecommerce sections."}
              </p>
              <p>
                {currentProduct.description ||
                  "The new composition prioritizes product identity, price clarity, trust signals, and merchant credibility above the fold. Supporting information is grouped into cleaner cards with stronger spacing and more consistent visual rhythm for Next.js production use."}
              </p>
            </div>
          </section>
        </div>

        <section className={cls.reviewSection} aria-labelledby={reviewHeadingId}>
          <div className={cls.sectionHead}>
                    ...
                    >
                      ...
                      <div className={cls.reviewContentWrap}>
                        ...
                        <p className={cls.reviewText}>
                          {review.content || "Exactly as described, with a solid buying experience."}
                        </p>
                      </div>
                    </article>
                  ))}
 ...
 * RegItem
 * ========================= */
export const SHOP_DETAIL_CENTERED: RegItem = {
  kind: "DetailCentered",
  label: "Detail Centered",
  defaults: {
    title: DEFAULT_TITLE,
    eyebrow: DEFAULT_EYEBROW,
    primaryCtaText: DEFAULT_PRIMARY_CTA,
  ...
  render: (p) => {
    const parsedProduct = safeJson<DetailCenteredProduct>(p.product);
    const productOverride = parsedProduct ? normalizeProduct(parsedProduct) || undefined : undefined;

    return (
      <div className="sectionContainer" aria-label="Shop Detail Centered">
        <DetailCentered
