...
import cls from "@/styles/templates/sections/Section/SectionSalesTwo.module.css";
...
export type SectionSalesTwoProps = {
  title?: string;
...
const DEFAULT_HIGHLIGHT = "Save up to 65%";
const DEFAULT_SUFFIX = "Live Now";
...
function normalizeProductItem(raw: unknown, index: number): SectionSalesProductItem | null {
  ...
  return {
    ...
    badge: toStringSafe(item.badge) || "Special Deal",
    price: Number.isFinite(price) ? price : undefined,
    originalPrice: Number.isFinite(originalPrice) && originalPrice > 0 ? originalPrice : undefined,
    discountPercent,
    soldText: soldCount > 0 ? `${formatCompactCount(soldCount)} sold recently` : "Popular now",
...
function getTrustText(item: SectionSalesProductItem): string {
  return item.shippingNote || "Secure checkout and fast delivery";
}

/* =========================
 * Component
 * ========================= */
export function SectionSalesTwo({
  ...
  preview = false,
  sectionAriaLabel = "Featured sale collection",
}: SectionSalesTwoProps) {
  ...
  const sectionHeadingId = `${regionId}-sales-two-heading`;
  const sectionDescId = `${regionId}-sales-two-description`;
  const liveStatusId = `${regionId}-sales-two-live-status`;
  ...
  useEffect(() => {
    ...
    async function fetchSalesProducts() {
      try {
        ...
        console.error("SectionSalesTwo fetch error:", error);
        setRemoteProducts([]);
      } finally {
        ...
        <header className={cls.hero}>
          <div className={cls.heroCopy}>
            <div className={cls.eyebrowRow}>
              <span className={cls.eyebrow}>Curated sale event</span>
              <span className={cls.eyebrowDivider} aria-hidden="true" />
              <span className={cls.eyebrowSubtle}>Updated bestsellers</span>
            </div>
            ...
            <div className={cls.trustRow} aria-label="Shopping benefits">
              <span className={cls.trustPill}>Fast shipping</span>
              <span className={cls.trustPill}>Secure checkout</span>
              <span className={cls.trustPill}>Easy returns</span>
            </div>
          </div>

          <div className={cls.heroAside}>
            <div className={cls.countdownCard}>
              ...
              <div className={cls.countdown} aria-label="Promotion countdown timer">
                <div className={cls.timeBlock}>
                  <span className={cls.timeValue}>{pad(timeLeft.days)}</span>
                  <span className={cls.timeLabel}>Days</span>
                </div>
                <div className={cls.timeBlock}>
                  <span className={cls.timeValue}>{pad(timeLeft.hours)}</span>
                  <span className={cls.timeLabel}>Hours</span>
                </div>
                <div className={cls.timeBlock}>
                  <span className={cls.timeValue}>{pad(timeLeft.minutes)}</span>
                  <span className={cls.timeLabel}>Mins</span>
                </div>
                <div className={cls.timeBlock}>
                  <span className={cls.timeValue}>{pad(timeLeft.seconds)}</span>
                  <span className={cls.timeLabel}>Secs</span>
                </div>
              </div>

              <span className={cls.liveText}>Limited stock on selected products</span>
            </div>
            ...
            <>
                    ...
                    {renderProductLink(
                      ...
                      <>
                        <div className={cls.featuredMedia}>
                          <div className={cls.featuredVisual}>
                            <Image
                          ...
                          <div className={cls.featuredBadges}>
                            {featuredItem.badge ? <span className={cls.saleBadge}>{featuredItem.badge}</span> : null}
                            {featuredItem.isNew ? <span className={cls.newBadge}>New</span> : null}
                            {getDiscountPercent(featuredItem) > 0 ? (
                              <span className={cls.discountBadge}>-{getDiscountPercent(featuredItem)}%</span>
                            ) : null}
                          </div>
                        </div>
                        ...
                        <div className={cls.featuredContent}>
                          <div className={cls.brandRow}>
                            <span className={cls.brandChip}>{featuredItem.brand || "Featured pick"}</span>
                          ...
                          <div className={cls.featuredPriceRow}>
                            {typeof featuredItem.price === "number" ? (
                              <div className={cls.featuredPrice}>{formatPrice(featuredItem.price)}</div>
                            ) : null}

                            <div className={cls.featuredMetaPrice}>
                              {typeof featuredItem.originalPrice === "number" ? (
                                <span className={cls.oldPrice}>{formatPrice(featuredItem.originalPrice)}</span>
                              ) : null}
                              {getDiscountPercent(featuredItem) > 0 ? (
                                <span className={cls.inlineDiscount}>Save {getDiscountPercent(featuredItem)}%</span>
                              ) : null}
                            </div>
                          </div>

                          {typeof getSavings(featuredItem) === "number" ? (
                            <div className={cls.savingsPill}>You save {formatPrice(getSavings(featuredItem))}</div>
                          ) : null}

                          <div className={cls.metaList}>
                            <span className={cls.metaItem}>{featuredItem.stockText || "In stock"}</span>
                            <span className={cls.metaDot} aria-hidden="true">
                              •
                            </span>
                            <span className={cls.metaItem}>{getTrustText(featuredItem)}</span>
                          </div>

                          <div className={cls.progressGroup} aria-label={featuredItem.soldText || "Popular now"}>
                            ...
                            <span className={cls.progressText}>{featuredItem.soldText || "Popular now"}</span>
                          </div>

                          <div className={cls.ctaRow}>
                            <span className={cls.primaryCta}>Shop this deal</span>
                            <span className={cls.secondaryCta}>View details</span>
                          </div>
                        </div>
                      </>,
                    )}
                          ...
                          <>
                            <div className={cls.sideThumb}>
                              <Image
                                ...
                                sizes="(max-width: 767px) 100vw, (max-width: 991px) 48vw, 22vw"
                              />
                              <div className={cls.sideThumbTop}>
                                <span className={cls.sideBrand}>{item.brand || "Top choice"}</span>
                                {discount > 0 ? <span className={cls.sideDiscount}>-{discount}%</span> : null}
                              </div>
                            </div>

                            <div className={cls.sideContent}>
                              <div className={cls.sideBadgeRow}>
                                <span className={cls.sideBadge}>{item.badge || "Special Deal"}</span>
                                {item.tag ? <span className={cls.sideTag}>{item.tag}</span> : null}
                              </div>
                              ...
                              {(typeof item.rating === "number" || typeof item.reviewCount === "number") && (
                                ...
                                >
                                  ...
                                  {typeof item.reviewCount === "number" ? (
                                    <span className={cls.sideReviews}>({formatCompactCount(item.reviewCount)})</span>
                                  ) : null}
                                </div>
                              )}
                              ...
                              <div className={cls.sideMeta}>
                                <span className={cls.sideMetaText}>{item.stockText || "In stock"}</span>
                                <span className={cls.sideMetaDot} aria-hidden="true">
                                  •
                                </span>
                                <span className={cls.sideMetaText}>{item.shippingNote || "Fast delivery"}</span>
                              </div>

                              <div className={cls.sideProgress} aria-label={item.soldText || "Popular now"}>
                                <div className={cls.sideProgressTrack}>
                                  <span className={cls.sideProgressFill} style={{ width: `${soldPercent}%` }} />
                                </div>
                              </div>
                            </div>
                          </>,
              ...
              <footer className={cls.footer}>
                ...
                <div className={cls.footerNote}>
                  <span className={cls.footerStrong}>Trusted shopping picks</span>
                  <span className={cls.footerText}>
                    Fresh deals selected for high value, strong reviews and clear savings
                  </span>
                </div>
              </footer>
            </>
 ...
 * RegItem
 * ========================= */
export const SHOP_SECTION_SALES_TWO: RegItem = {
  kind: "SectionSalesTwo",
  label: "Section Sales Two",
  defaults: {
    highlightText: DEFAULT_HIGHLIGHT,
    discountText: DEFAULT_SUFFIX,
  ...
  inspector: [
    ...
    { key: "discountText", label: "Suffix / status", kind: "text" },
  ...
  render: (p) => {
    ...
    return (
      <div className="sectionContainer" aria-label="Shop Section Sales Two">
        <SectionSalesTwo
...
export default SectionSalesTwo;
