...
import cls from "@/styles/templates/sections/Section/SectionSalesThree.module.css";
...
export type SectionSalesThreeProps = {
  title?: string;
...
const DEFAULT_TITLE = "Flash Sale Collection";
const DEFAULT_HIGHLIGHT = "Best deals for smart shoppers";
const DEFAULT_SUFFIX = "Limited-time savings";
 ...
 * Component
 * ========================= */
export function SectionSalesThree({
  title = DEFAULT_TITLE,
  ...
  preview = false,
  sectionAriaLabel = "Flash sale products",
}: SectionSalesThreeProps) {
  ...
  const sectionHeadingId = `${regionId}-sales-three-heading`;
  const sectionDescId = `${regionId}-sales-three-description`;
  const liveStatusId = `${regionId}-sales-three-live-status`;
  ...
  useEffect(() => {
    ...
    async function fetchSalesProducts() {
      try {
        ...
        console.error("SectionSalesThree fetch error:", error);
        setRemoteProducts([]);
      } finally {
      ...
      <div className={cls.shell}>
        <header className={cls.hero}>
          <div className={cls.heroContent}>
            <div className={cls.heroTop}>
              <span className={cls.kicker}>Editor’s Pick Sale</span>
              <span className={cls.kickerDivider} aria-hidden="true" />
              <span className={cls.kickerSubtle}>{discountText}</span>
            </div>
            ...
            <div className={cls.benefits} aria-label="Shopping highlights">
              <span className={cls.benefitPill}>Verified demand</span>
              <span className={cls.benefitPill}>Fast delivery</span>
              <span className={cls.benefitPill}>Clear savings</span>
            </div>
          </div>

          <aside className={cls.heroPanel}>
            <div className={cls.countdownCard}>
              <div className={cls.countdownTop}>
                <span className={cls.countdownEyebrow}>Ends soon</span>
                <span className={cls.liveIndicator}>
              ...
              <div className={cls.panelFooter}>
                <span className={cls.panelFooterText}>High-conversion picks refreshed automatically</span>
              </div>
            </div>
        ...
        <div className={cls.body}>
          {loading && items.length === 0 ? (
            ...
            <>
              <div className={cls.contentGrid} role="list" aria-describedby={liveStatusId}>
                {featuredItem ? (
                  <article className={cls.featuredCard} role="listitem">
                    {renderProductLink(
                      ...
                      cls.featuredLink,
                      <>
                        <div className={cls.featuredVisualWrap}>
                          <div className={cls.featuredGlow} aria-hidden="true" />
                          ...
                          <div className={cls.featuredBadges}>
                            {featuredItem.badge ? <span className={cls.primaryBadge}>{featuredItem.badge}</span> : null}
                            {featuredItem.isNew ? <span className={cls.newBadge}>New Arrival</span> : null}
                            {getDiscountPercent(featuredItem) > 0 ? (
                              <span className={cls.discountBadge}>Save {getDiscountPercent(featuredItem)}%</span>
                            ) : null}
                          </div>
                        </div>

                        <div className={cls.featuredBody}>
                          <div className={cls.featuredHeader}>
                            <div className={cls.featuredMetaTop}>
                          ...
                          <div className={cls.featuredPricing}>
                            {typeof featuredItem.price === "number" ? (
                              <span className={cls.featuredPrice}>{formatPrice(featuredItem.price)}</span>
                            ) : null}

                            <div className={cls.pricingMeta}>
                              ...
                              {typeof getSavings(featuredItem) === "number" ? (
                                <span className={cls.savingsText}>
                          ...
                          <div className={cls.progressBlock} aria-label={featuredItem.soldText || "Trending now"}>
                            <div className={cls.progressMeta}>
                              <span className={cls.progressLabel}>Demand</span>
                          ...
                          <div className={cls.featuredActions}>
                            <span className={cls.primaryCta}>Shop now</span>
                            <span className={cls.secondaryCta}>See details</span>
                          </div>
                        </div>
                      </>,
                    )}
                          ...
                          <>
                            <div className={cls.sideMedia}>
                              <Image
                                ...
                                sizes="(max-width: 767px) 100vw, (max-width: 991px) 48vw, 24vw"
                              />

                              <div className={cls.sideOverlay}>
                                <span className={cls.sideBrand}>{item.brand || "Top Pick"}</span>
                                {discount > 0 ? <span className={cls.sideDiscount}>-{discount}%</span> : null}
                              </div>
                            </div>

                            <div className={cls.sideBody}>
                              <div className={cls.sideBadges}>
                                <span className={cls.sideBadge}>{item.badge || "Deal"}</span>
                              ...
                              {(typeof item.rating === "number" || typeof item.reviewCount === "number") && (
                                ...
                                >
                                  ...
                                  {typeof item.reviewCount === "number" ? (
                                    <span className={cls.sideReviews}>
              ...
              <footer className={cls.footer}>
                ...
                <div className={cls.footerSummary}>
                  <div className={cls.footerCard}>
                    <span className={cls.footerStat}>Smart discounts</span>
                    <span className={cls.footerText}>
                      Products with visible price advantage and strong presentation.
                    </span>
                  </div>
                  <div className={cls.footerCard}>
                    <span className={cls.footerStat}>Trust-first UX</span>
                    <span className={cls.footerText}>
                      Ratings, stock, delivery notes, and demand cues improve conversion.
                    </span>
                  </div>
                </div>
              </footer>
 ...
 * RegItem
 * ========================= */
export const SHOP_SECTION_SALES_THREE: RegItem = {
  kind: "SectionSalesThree",
  label: "Section Sales Three",
  defaults: {
    title: DEFAULT_TITLE,
    highlightText: DEFAULT_HIGHLIGHT,
    discountText: DEFAULT_SUFFIX,
  ...
  render: (p) => {
    ...
    return (
      <div className="sectionContainer" aria-label="Shop Section Sales Three">
        <SectionSalesThree
...
export default SectionSalesThree;
