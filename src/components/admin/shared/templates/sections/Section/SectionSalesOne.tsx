...
import cls from "@/styles/templates/sections/Section/SectionSalesOne.module.css";
...
export type SectionSalesOneProps = {
  title?: string;
...
const DEFAULT_TITLE = "Limited-Time Gift Picks";
const DEFAULT_HIGHLIGHT = "Up to 65% Off";
const DEFAULT_SUFFIX = "⚡";
const DEFAULT_NOTE = "Extra savings on selected bestsellers";
 ...
 * Component
 * ========================= */
export function SectionSalesOne({
  title = DEFAULT_TITLE,
  ...
  const sectionHeadingId = `${regionId}-sales-heading`;
  const sectionDescId = `${regionId}-sales-description`;
  const liveStatusId = `${regionId}-sales-live-status`;
  ...
  useEffect(() => {
    ...
    async function fetchSalesProducts() {
      try {
        ...
        console.error("SectionSalesOne fetch error:", error);
        setRemoteProducts([]);
      } finally {
      ...
      <div className={cls.inner}>
        <header className={cls.head}>
          <div className={cls.headLeft}>
            ...
            <div className={cls.kickerRow} id={sectionDescId}>
              <span className={cls.kicker}>Limited-time campaign</span>
              <span className={cls.kickerDivider} aria-hidden="true" />
              <span className={cls.kickerSubtle}>{noteText}</span>
            </div>
          </div>
 ...
 * RegItem
 * ========================= */
export const SHOP_SECTION_SALES_ONE: RegItem = {
  kind: "SectionSalesOne",
  label: "Section Sales One",
  defaults: {
    title: DEFAULT_TITLE,
    highlightText: DEFAULT_HIGHLIGHT,
    discountText: DEFAULT_SUFFIX,
  ...
  render: (p) => {
    ...
    return (
      <div className="sectionContainer" aria-label="Shop Section Sales One">
        <SectionSalesOne
...
export default SectionSalesOne;
