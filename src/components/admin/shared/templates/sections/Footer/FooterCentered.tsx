...
/* ================= Types ================= */
export type FooterCenteredInfoItem = { label: string; text: string };

export type FooterCenteredLinkItem = { label: string; href: string };

export type FooterCenteredHotlineItem = { label: string; phone: string };

export type FooterCenteredSocialItem = { label: string; href: string; icon: string };

export type FooterCenteredPartnerItem = { label: string; href: string; imageSrc: string };

export type FooterCenteredCert = {
  imageSrc: string;
  title: string;
  sub: string;
};

export type FooterCenteredBrand = {
  name: string;
  tag: string;
  info: FooterCenteredInfoItem[];
  cert?: FooterCenteredCert;
};

export type FooterCenteredProps = {
  brand?: FooterCenteredBrand;

  linksTitle?: string;
  links?: FooterCenteredLinkItem[];
  ...
  supportTitle?: string;
  hotlines?: FooterCenteredHotlineItem[];
  socials?: FooterCenteredSocialItem[];

  partnersTitle?: string;
  partners?: FooterCenteredPartnerItem[];
...
/* ================= Defaults ================= */
const DEFAULT_BRAND: FooterCenteredBrand = {
...
const DEFAULT_LINKS: FooterCenteredLinkItem[] = [
...
export function FooterCentered({
  ...
  linksTitle = "Explore",
  links,
  newsletterTitle = "Stay close with us",
  newsletterDesc = "Subscribe to receive gentle updates, new arrivals and special offers curated for you.",
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    setEmail("");
    showToast("Subscribed! We will keep in touch.");
  };
  ...
  return (
    <footer className={cls.siteFooter} aria-label="Site footer">
      <div className={cls.footerGlow} aria-hidden="true" />
        ...
        <div className={cls.footerContainer}>
          ...
          <div className={cls.footerMiddleGrid}>
            ...
            <div className={cls.footerCard}>
              <h3 className={cls.footerCardTitle}>{supportTitle}</h3>
...
export const SHOP_FOOTER_GREEN_CENTERED: RegItem = {
  ...
  render: (p) => {
    const brand = safeJson<FooterCenteredBrand>(p.brand);
    const links = safeJson<FooterCenteredLinkItem[]>(p.links);
    const hotlines = safeJson<FooterCenteredHotlineItem[]>(p.hotlines);
    const socials = safeJson<FooterCenteredSocialItem[]>(p.socials);
    const partners = safeJson<FooterCenteredPartnerItem[]>(p.partners);

    return (
      <div className="sectionContainer" aria-label="Shop Footer (Centered)">
        <FooterCentered
          brand={brand}
          linksTitle={String(p.linksTitle || "Explore")}
          links={links}
          newsletterTitle={String(p.newsletterTitle || "Stay close with us")}
          ...
          supportTitle={String(p.supportTitle || "Need help?")}
          hotlines={hotlines}
          socials={socials}
          partnersTitle={String(p.partnersTitle || "Our trusted partners")}
          partners={partners}
