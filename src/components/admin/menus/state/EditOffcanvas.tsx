...
export default function EditOffcanvas({ item, onClose }: Props) {
    ...
    const resolveDraftTitle = useCallback(
        (menuItem: BuilderMenuItem) => {
            const page = menuItem.internalPageId
                ? pagesById.get(menuItem.internalPageId)
                : undefined;
    ...
    const hrefPreview = useMemo(() => {
        ...
        if (safeDraft.linkType === 'internal') {
            ...
            const page = safeDraft.internalPageId
                ? pagesById.get(safeDraft.internalPageId)
                : undefined;
            return (
            ...
            >
                    ...
                    <div className={styles.ocHeaderLeft}>
                        ...
                        <div>
                            ...
                            <div className={styles.ocSub}>
                                {t('menus.edit.menuSet')}: <b>{currentSet}</b> • ID:{' '}
                                <code>{String(safeDraft.id)}</code>
                            </div>
                        </div>
                    </div>
                ...
                <div className={styles.ocBody}>
                                ...
                                <select
                                    ...
                                    onChange={(event) =>
                                        handleLinkTypeChange(
                                            event.target.value as BuilderMenuItem['linkType'],
                                        )
                                    }
                                >
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(event) => {
                                            ...
                                            if (
                                                !currentPath ||
                                                currentPath === resolvePathForPatch(safeDraft)
                                            ) {
                                                setPathInput(value);
                                            }
                                        }}
                                    ...
                                    {(safeDraft.schedules || []).length === 0 ? (
                                        ...
                                        <div className={styles.scheduleList}>
                                            {(safeDraft.schedules || []).map(
                                                (
                                                    schedule: { when: string; url: string },
                                                    index: number,
                                                ) => (
                                                    <div
                                                    ...
                                                    >
                                                        <div className={styles.scheduleIdx}>
                                                            {index + 1}
                                                        </div>
                    ...
                    <section className={styles.panel}>
                        ...
                        <div>
                            <label className={styles.formLabel}>
                                {t('menus.edit.pathSavedToDatabase')}
                            </label>
                            <input
                                className={styles.formControl}
                                placeholder={
