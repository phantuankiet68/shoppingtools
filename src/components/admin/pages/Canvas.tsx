            ...
            const renderProps =
                b.kind === 'Row'
                    ? {
                          ...
                          _selfId: b.id,
                          _colCounts: rowColCounts.get(b.id),
                          _device: device,
                      }
                    : b.kind === 'Section'
                      ? {
    ...
    const getDropMeta = React.useCallback((ev: React.DragEvent) => {
        ...
        if (secEl) {
            const parentSectionId =
                secEl.dataset.hostId || secEl.dataset.hostid || secEl.dataset.sectionId;
                    ...
                    {rootBlocks.length === 0 && (
                        <div className={`${cls.empty}`}>
                            <div className={cls.emptyCard}>
                                ...
                                <div className={cls.emptyHint}>
                                    Thả component từ palette sang để bắt đầu thiết kế ✨
                                </div>
                            </div>
                        </div>
                    )}
