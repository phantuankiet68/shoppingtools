...
function PageList({
    ...
    const handleOpenCreateModal = useCallback(() => {
        if (limitReached) {
            modal.error(
                'Limit Reached',
                `You have reached the maximum number of pages (${maxPages}) for this workspace.`,
            );
        ...
        <aside className={styles.leftPane}>
            ...
            <div className={styles.listWrap} id="listWrap">
                {!loading && pages.length === 0 && (
                    <div className={styles.empty}>{t('pageList.noResults')}</div>
                )}
                {pages.map((p) => {
                    const ts = p.updatedAt || p.createdAt || 0;
                    const dateText = ts
                        ? dateFormatter.format(new Date(ts))
                        : t('pageList.common.noDate');
                    return (
                        <button
                            key={p.id}
                            type="button"
                        ...
                        >
                            <div className={styles.itemIcon}>
                                {initialsFromTitle(p.title || p.slug)}
                            </div>
                            <div className={styles.itemMain}>
                                <div className={styles.itemTitle}>
                                    <span className={styles.titleText}>
                                        {p.title || t('pageList.untitled')}
                                    </span>
                                    <span
