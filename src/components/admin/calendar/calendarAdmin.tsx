...
type BookingStatus = "pending" | "confirmed" | "checked_in" | "in_service" | "completed" | "cancelled" | "no_show";
...
export default function CalendarAdmin() {
  const { user, sites: authSites } = useAdminAuth();
  ...
  const [calendarTitle, setCalendarTitle] = useState(
    formatCalendarTitle(initialDateRef.current, "timeGridWeek", currentLocale),
  );
  const [search, setSearch] = useState("");
  const [selectedBookingId, setSelectedBookingId] = useState<string | null>(null);
  const [miniCalendarDate, setMiniCalendarDate] = useState<Date>(initialDateRef.current);
  const [selectedMiniDay, setSelectedMiniDay] = useState<number>(initialDateRef.current.getDate());
  ...
  const [editForm, setEditForm] = useState<CreateBookingForm>({
    customerName: "",
    customerPhone: "",
    serviceName: "",
    source: "website",
  ...
  const loadBookings = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(tc("errors.invalidServerResponse"));
      }
  ...
  const handleEditFormChange = (field: keyof CreateBookingForm, value: string) => {
    setEditForm((prev) => ({
      ...prev,
      [field]: value,
    }));
  ...
  const handleUpdateBooking = async () => {
    ...
    try {
      ...
      setBookings((prev) => prev.map((booking) => (booking.id === selectedBooking.id ? result.data : booking)));
  ...
  const todayBookings = useMemo(() => {
    ...
    return bookings.filter((booking) => isSameDate(new Date(booking.start), today));
  }, [bookings]);
  ...
  const handleSelectSlot = (arg: DateSelectArg) => {
    ...
    const endIso = arg.end ? arg.end.toISOString() : addMinutesToIsoString(startIso, 60);
  ...
  const handleCreateFormChange = (field: keyof CreateBookingForm, value: string) => {
    setCreateForm((prev) => ({
      ...prev,
      [field]: value,
    }));
  ...
  const handleCreateBooking = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!phoneRegex.test(createForm.customerPhone.trim())) {
      setError(tc("errors.invalidPhone"));
      return;
    }
    if (!siteId) {
      setError(tc("errors.siteNotFound"));
    ...
    try {
      ...
      if (new Date(createForm.end) <= new Date(createForm.start)) {
        setError(tc("errors.invalidTime"));
        return;
      }
  ...
  const handleDeleteBooking = async () => {
    ...
    const confirmed = window.confirm(tc("confirm.deleteBooking", { name: selectedBooking.customerName }));
    ...
    try {
      ...
      setBookings((prev) => {
        const remaining = prev.filter((booking) => booking.id !== selectedBooking.id);
  ...
  const handleApplySuggestion = (field: "serviceName" | "note", value: string) => {
    setCreateForm((prev) => {
      ...
      if (field === "note") {
        const hasValue = currentValue.toLowerCase().includes(value.toLowerCase());
  ...
  return (
    <div className={styles.pageShell}>
            ...
            <div className={styles.bookingQuickList}>
              {nearestBookings.length > 0 ? (
                nearestBookings.map((booking) => {
                  const isUpcoming = new Date(booking.start).getTime() >= Date.now();
                  ...
                  return (
                    <button
                      key={booking.id}
                      type="button"
                      className={`${styles.bookingQuickItem} ${isActive ? styles.bookingQuickItemActive : ""}`}
                      onClick={() => setSelectedBookingId(booking.id)}
                    >
                      <div className={styles.bookingQuickTop}>
                        <div className={styles.bookingQuickName}>{booking.customerName}</div>

                        <span
                          className={`${styles.bookingQuickStatus} ${
                            isUpcoming ? styles.bookingQuickStatusUpcoming : styles.bookingQuickStatusRecent
                          }`}
                        >
                      ...
                      <div className={styles.bookingQuickService}>{booking.serviceName}</div>
          ...
          <header className={styles.topbar}>
            ...
            <div className={styles.topbarCenter}>
              ...
              <div className={styles.viewTabs}>
                ...
                <button
                  type="button"
                  className={`${styles.viewTab} ${currentView === "timeGridWeek" ? styles.viewTabActive : ""}`}
            ...
            <div className={styles.topbarRight}>
              ...
              <button type="button" className={styles.todayBtn} onClick={() => void loadBookings()}>
                {tc("actions.reload")}
              </button>
            </div>
          </header>
          ...
          <div className={styles.contentGrid}>
            <div className={styles.calendarSurface}>
              <div className={styles.calendarToolbarInfo}>
                ...
                <div>
                  <button type="button" className={styles.dayOffChip} onClick={() => void loadBookings()}>
              ...
              <div className={styles.calendarWrap}>
                <FullCalendar
                  ...
                  datesSet={(arg: DatesSetArg) => {
                    ...
                    setCalendarTitle(formatCalendarTitle(currentDate, nextView, currentLocale));
              ...
              <div className={styles.card}>
                <div className={styles.cardHead}>
                  <div>
                    ...
                    <div className={styles.cardSub}>{loading ? tc("loading.loadingData") : tc("details.subtitle")}</div>
                  </div>
                ...
                {!error && loading ? <div className={styles.emptyState}>{tc("loading.loadingBookings")}</div> : null}
                    ...
                    {!isEditing ? (
                      <>
                        ...
                        <div className={styles.actionRow}>
                          <button type="button" className={styles.primaryBtn} onClick={handleStartEdit}>
                      ...
                      <>
                        <div className={styles.bookingFormGrid}>
                          <div className={styles.bookingField}>
                            <label className={styles.bookingLabel}>{tc("details.customer")}</label>
                          ...
                          <div className={styles.bookingField}>
                            <label className={styles.bookingLabel}>{tc("details.phone")}</label>
                          ...
                          <div className={styles.bookingField}>
                            <label className={styles.bookingLabel}>{tc("details.service")}</label>
                          ...
                          <div className={styles.bookingField}>
                            ...
                            <input
                              type="datetime-local"
                              value={toDateTimeLocalValue(editForm.start)}
                              onChange={(e) => handleEditFormChange("start", fromDateTimeLocalValue(e.target.value))}
                              className={styles.bookingInput}
                            />
                          </div>
                          ...
                          <div className={styles.bookingField}>
                            ...
                            <input
                              type="datetime-local"
                              value={toDateTimeLocalValue(editForm.end)}
                              onChange={(e) => handleEditFormChange("end", fromDateTimeLocalValue(e.target.value))}
                              className={styles.bookingInput}
                            />
                          </div>
      ...
      {isCreateModalOpen ? (
        <div
          className={styles.bookingModalOverlay}
          onClick={(e) => {
          ...
          <div className={styles.bookingModal} onClick={(e) => e.stopPropagation()}>
            <div className={styles.bookingModalHeader}>
              <div>
                ...
                <p className={styles.bookingModalSubtitle}>{tc("modal.createSubtitle")}</p>
              </div>

              <button type="button" onClick={handleCloseCreateModal} className={styles.bookingModalClose}>
                ✕
              </button>
            </div>
            ...
            <form onSubmit={handleCreateBooking} className={styles.bookingForm}>
              <div className={styles.bookingFormGrid}>
                ...
                <div className={`${styles.bookingField} ${styles.bookingFieldFull}`}>
                  ...
                  <input
                    ...
                    onChange={(e) => handleCreateFormChange("serviceName", e.target.value)}
                ...
                <div className={styles.bookingField}>
                  ...
                  <input
                    ...
                    onChange={(e) => handleCreateFormChange("start", fromDateTimeLocalValue(e.target.value))}
                    required
                    className={styles.bookingInput}
                  />
                </div>
                ...
                <div className={styles.bookingField}>
                  ...
                  <input
                    ...
                    onChange={(e) => handleCreateFormChange("end", fromDateTimeLocalValue(e.target.value))}
                    required
                    className={styles.bookingInput}
                  />
                </div>
                ...
                <div className={styles.bookingField}>
                  ...
                  <select
                    ...
                    onChange={(e) => handleCreateFormChange("source", e.target.value as BookingSource)}
                    className={styles.bookingInput}
                  >
              ...
              <div className={styles.bookingFormActions}>
                <button type="button" onClick={handleCloseCreateModal} className={styles.bookingCancelBtn}>
                  <i className="bi bi-x-circle me-2"></i>
                  {tc("actions.cancel")}
                </button>

                <button type="submit" disabled={createLoading} className={styles.bookingSubmitBtn}>
                  {createLoading ? (
                    <>
                      <i className="bi bi-arrow-repeat spin me-2"></i>
                      {tc("loading.creating")}
