import { SEO_CATEGORY_CONFIG } from '@/constants/pages/seoConstants';
...
export function fillAutoSEO(
    ...
    const categoryConfig = SEO_CATEGORY_CONFIG[category as keyof typeof SEO_CATEGORY_CONFIG];

    const keywords = categoryConfig?.keywordsKey
        ? t(categoryConfig.keywordsKey)
              .split(',')
              .map((item) => item.trim())
        : [];

    const categoryDescription = categoryConfig?.descriptionKey
        ? t(categoryConfig.descriptionKey)
        : '';

    const categoryOgDescription = categoryConfig?.ogDescriptionKey
        ? t(categoryConfig.ogDescriptionKey)
        : '';
    ...
    const metaDescription = (
        isHome
            ? `${siteName} - ${
                  categoryDescription ?? buildMetaDescription(t, title, siteName, isHome)
              }`
            : `${title} - ${
                  categoryDescription ?? buildMetaDescription(t, title, siteName, isHome)
              }`
    ...
    const ogDescription = (
        categoryOgDescription || buildOgDescription(t, title, siteName, isHome)
    ).slice(0, 200);
    ...
    return {
        seo: {
            ...
            focusKeyword: [title, ...keywords, category, siteName].filter(Boolean).join(', '),
            ogTitle,
            ogDescription,
