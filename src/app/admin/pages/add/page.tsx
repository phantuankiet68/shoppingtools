...
import { Canvas, ControlsPalette, Inspector } from '@/components/admin/pages';
...
import {
    ...
    publishAdminPage,
    saveAdminPage,
} from '@/services/pages/add/adminPages.service';
...
export default function UiBuilderAddPage() {
        ...
        (e: React.DragEvent) => {
            ...
            const droppedTpl = (() => {
                ...
                try {
                    ...
                    const draggedTemplateId: string = (
                        payload?.templateId || txt.replace('template:', '')
                    ).trim();
    ...
    const savePage = React.useCallback(async () => {
        ...
        try {
            ...
            const { safeTitle, finalSlug, finalPath } = normalizeSlugAndPath(
                state.slug,
                state.title,
            );
