...
export async function GET(req: Request) {
  try {
    ...
    const url = new URL(req.url);
    const takeParam = Number(url.searchParams.get("take") ?? 8);
    const take = Number.isFinite(takeParam) ? Math.min(Math.max(takeParam, 1), 24) : 8;
    ...
    return NextResponse.json({
      ...
      items: items.map((p) => ({
        ...
        originalPrice: p.marketPrice?.toString() ?? null,
        savingPrice: p.savingPrice?.toString() ?? null,
        productQty: p.productQty,
        stockText:
          typeof p.productQty === "number"
            ? p.productQty <= 0
              ? "Out of stock"
              : p.productQty <= 10
                ? `Only ${p.productQty} left`
                : "In stock"
            : "In stock",
        shippingNote: "Fast delivery",
        ...
        image: p.images[0]
          ...
          : null,
        imageSrc: p.images[0]?.imageUrl ?? "/images/placeholder-product.png",
      })),
      total: items.length,
    });
  } catch (error: unknown) {
