...
function buildVariantWhere(params: {
    productId: string;
    q: string;
    active: string;
}): VariantListWhere {
    ...
    try {
        ...
        if (existed) {
            if (existed.deletedAt) {
                return NextResponse.json(
                    {
                        error: 'SKU already exists in this site (the old variant is soft-deleted). Please use another SKU or restore/update the old variant.',
                    },
                    { status: 409 },
                );
            }
