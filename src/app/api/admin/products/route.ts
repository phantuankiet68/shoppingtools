...
export async function POST(req: NextRequest) {
    try {
        ...
        if (brandInput) {
            ...
            if (!brand) {
                return NextResponse.json(
                    {
                        message:
                            'Brand not found (provide valid brand id, slug, or name for this site)',
                    },
                    { status: 400 },
                );
            }
        ...
        const imageMedia = (body.media ?? []).filter(
            (m) => m.type === 'image' && String(m.url ?? '').trim(),
        );
        ...
        if (
            ...
            return NextResponse.json(
                { message: 'Duplicate unique value (slug or other unique field).' },
                { status: 409 },
            );
        }
