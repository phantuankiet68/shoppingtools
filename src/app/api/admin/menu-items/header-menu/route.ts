...
// Cho phép locale "all" (để khỏi bị rỗng items)
type LocaleParam = Locale | "all";

const DEFAULT_SET_KEY: MenuSetKey = "home"; // ✅ header menu của trang Home
const DEFAULT_LOCALE: LocaleParam = "en";
...
export async function GET(req: Request) {
  try {
    ...
    // locale: "en" | "vi" | ... | "all"
    const localeParam = (url.searchParams.get("locale") as LocaleParam) || DEFAULT_LOCALE;
    ...
    const where: Prisma.MenuItemWhereInput = {
      ...
      ...(localeParam === "all" ? {} : { locale: localeParam }),
    };
