...
function normalizeMenuKey(value: unknown) {
    return String(value ?? '')
        .trim()
        .toLowerCase()
        .normalize('NFD')
...
export async function GET(req: Request) {
    try {
        ...
        const parentId =
            parentIdRaw && parentIdRaw !== 'null' && parentIdRaw !== '' ? parentIdRaw : undefined;
        ...
        if (lite) {
            ...
            return NextResponse.json(
                items.map((m) => ({ id: m.id, label: m.title, path: m.path ?? '/' })),
            );
        }
        ...
        console.error('GET /api/admin/menus error:', e);
...
export async function POST(req: Request) {
    try {
        ...
        const area: MenuArea = isMenuArea(body.area) ? body.area : 'SITE';

        if (!title) {
            return NextResponse.json({ error: { title: 'Title is required' } }, { status: 400 });
        }

        const key = normalizeMenuKey(body.key ?? title);

        if (!key) {
            return NextResponse.json({ error: { key: 'Key is required' } }, { status: 400 });
        }
        ...
        if (body.parentId) {
            ...
            if (!parent || parent.siteId !== siteId) {
                return NextResponse.json(
                    { error: { parentId: 'Parent not found in the same site' } },
                    { status: 400 },
                );
            }
        }
        ...
        console.error('POST /api/admin/menus error:', e);
        return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
    }
}
