...
export async function GET(_req: Request, ctx: Ctx) {
  try {
    ...
    console.error("GET /api/admin/menu-items/[id] error:", e);
...
export async function PATCH(req: Request, ctx: Ctx) {
  try {
    ...
    // ✅ PRECHECK: tránh Prisma P2025 spam log
    ...
    console.error("PATCH /api/admin/menus/[id] error:", e);
    return NextResponse.json({ error: "Internal Server Error" }, { status: 500 });
  }
}
...
export async function DELETE(_req: Request, ctx: Ctx) {
  try {
    ...
    // ✅ PRECHECK để trả 404 sạch, tránh P2025
    const exists = await prisma.menuItem.findUnique({ where: { id }, select: { id: true } });
    if (!exists) return NextResponse.json({ error: "Not found" }, { status: 404 });
    ...
    console.error("DELETE /api/admin/menus/[id] error:", e);
    return NextResponse.json({ error: "Internal Server Error" }, { status: 500 });
  }
}
