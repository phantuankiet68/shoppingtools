...
export async function PUT(req: Request, context: RouteContext) {
    try {
        ...
        const duplicatedSlug = await prisma.brand.findFirst({
            where: {
                siteId,
                slug,
                NOT: {
        ...
        if (duplicatedSlug) {
            return NextResponse.json(
                { error: 'Slug already exists in this site' },
                { status: 409 },
            );
        }

        const duplicatedName = await prisma.brand.findFirst({
            where: {
                siteId,
                name,
                NOT: {
        ...
        if (duplicatedName) {
            return NextResponse.json(
                { error: 'Name already exists in this site' },
                { status: 409 },
            );
        }
