...
export async function DELETE(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
    try {
        ...
        const childCount = await prisma.imageFolder.count({
            where: { userId: user.id, parentId: id },
        });
        if (childCount > 0) {
            return NextResponse.json(
                { error: 'Folder has subfolders. Delete/move subfolders first.' },
                { status: 409 },
            );
        }

        // images in folder
        const imgCount = await prisma.imageAsset.count({
        ...
        if (mode === 'restrict') {
            if (imgCount > 0) {
                return NextResponse.json(
                    { error: 'Folder is not empty. Move/delete images first.' },
                    { status: 409 },
                );
            }
