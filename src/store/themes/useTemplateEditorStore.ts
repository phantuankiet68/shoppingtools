...
import {
    ...
    guessCssModulePath,
    normalizeForScope,
} from '@/services/themes/templateFiles.service';
...
export function useTemplateEditorStore() {
    ...
    const isDirty = useMemo(
        () => tsx !== originalRef.current.tsx || css !== originalRef.current.css,
        [tsx, css],
    );
    ...
    const grouped = useMemo(() => {
        ...
        for (const x of items) {
            ...
            if (q && !(x.name.toLowerCase().includes(q) || x.path.toLowerCase().includes(q)))
                continue;
...
`;
    ...
    const deleteTemplate = useCallback(async () => {
        ...
        if (tsxPath.includes('/shopGreen/')) {
            setToast({
                type: 'err',
                msg: 'Không cho xóa shopGreen vì đang được import tĩnh (sẽ vỡ build).',
            });
            return;
        }
